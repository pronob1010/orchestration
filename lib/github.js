...
async function createGitHubIssue(payload) {
  ...
  // Try gh CLI first.
  // Note: `gh issue create` does NOT support --json; it outputs the issue URL as plain text.
  ...
  if (ghResult.ok) {
    // stdout is the new issue URL, e.g. https://github.com/owner/repo/issues/42
    const url = (ghResult.result.stdout || '').trim();
  ...
  if (!process.env.GITHUB_TOKEN) {
    const detail = (ghResult.result?.stderr || ghResult.error || '').split('\n')[0];
    throw new Error(`gh CLI issue create failed: ${detail}. Set GITHUB_TOKEN in .env to use the REST API fallback.`);
  }
